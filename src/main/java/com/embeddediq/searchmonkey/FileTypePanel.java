 ...
 * @author cottr
 */
public class FileTypePanel extends javax.swing.JPanel implements ListSelectionListener {

    private final MimeType[] values;
    /**
    ...
    public FileTypePanel() {
        ...
        DefaultListModel model = new DefaultListModel(); // (DefaultListModel)jList1.getModel();
        model.removeAllElements();
        for (MimeType mime: values)
        {
            model.addElement(mime.toString().replace("$", "") + " - " + mime.contentType);
        }
    ...
    {
        if (e.getValueIsAdjusting()) return; // Do nothing
        
        List<String> items = new ArrayList<>();
        for (int i: jList1.getSelectedIndices())
        {
            items.add(values[i].contentType);
        }
        ...
        this.jTextField1.setText(String.join(", ", items));
    }
    ...
    {
        if (init.isActive) {
            jTextField1.setText(init.mimeName);
        }
    }
    
    public FileTypeEntry get()
    {
        return new FileTypeEntry(jTextField1.getText());
    }
